...
# --- Шаг 1 ---
Write-Host "`n🔨 Шаг 1: Сборка веб-приложения..." -ForegroundColor Cyan
npm run build
if ($LASTEXITCODE -ne 0) { Write-Host "❌ Ошибка сборки!" -ForegroundColor Red; exit 1 }

# --- Шаг 2 ---
Write-Host "`n📱 Шаг 2: Синхронизация Capacitor..." -ForegroundColor Cyan
npx cap sync android
if ($LASTEXITCODE -ne 0) { Write-Host "❌ Ошибка синхронизации!" -ForegroundColor Red; exit 1 }

# --- Шаг 3 ---
Write-Host "`n🏗️ Шаг 3: Сборка APK через Gradle..." -ForegroundColor Cyan
...
# --- Шаг 4 ---
Write-Host "`n📐 Шаг 4: Выравнивание (zipalign)..." -ForegroundColor Cyan
...
# --- Шаг 5 ---
Write-Host "`n🔑 Шаг 5: Подпись APK (apksigner)..." -ForegroundColor Cyan
...
# --- Готово ---
Write-Host "`n✅ Готово! Подписанный APK: $SignedApk" -ForegroundColor Green
Write-Host "📂 Размер: $([math]::Round((Get-Item $SignedApk).Length / 1MB, 2)) MB" -ForegroundColor Green
